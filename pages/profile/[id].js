...
const Profile = ({ profile }) => {
  ...
  const profileIntro = profile.introduction || `${profile.displayName} has chosen to remain mysterious about themselves.`
  const profileInterests = JSON.stringify(profile.interests) || `${profile.displayName} has a number of interests, but have chosen not to share with anyone.`
    ...
    <div className="container">
      ...
      <main>
        ...
        <div className="profileHead">
          {
            viewingMyOwn ?
              <div className="profileParticulars">
                <div><img className='profileImage' id='profileAvatar' src={profile.avatarSource}></img></div>
                <div className='profileImageCover'>
                  <FileDropZone 
                    className="form-group file-area"
                    onChange={async(evt) => {
                      console.log('evt.target', evt)
                      const selectedFile = document.getElementById('newAvatar').files[0]
                      const storageRef = firebase.storage().ref()
                      const profileAvatarRef = storageRef.child(`images/avatars/${user.id}`)
                  ...
                  >
                    <input type="file" accept="image/*" name="newAvatar" id="newAvatar" required="required" title="Click or drag an image for your profile photo"/>
                    <EditIcon className="icon" />
                    <div className="file-dummy">
                      <div className="success">Your photo is selected. Please wait ...</div>
            ...
            :
              <div className="profileParticulars-notmine">
                <div><img className='profileImage' src={profile.avatarSource}></img></div> 
              </div>
          }
        </div>
        ...
        <h3 className="profileSectionHead">Introduction</h3>
        {
          ...
          :
          <div className="profileParticulars-notmine">
            <div className="profileText"><p>{profileIntro}</p></div>
          </div>
        }
        ...
        { 
          profile && (profile.paths.length > 0 || profile.pathsTaken.length > 0)  ?
          <div className="profileParticulars">
            ...
            <ul>
              {profile.paths.map((p) => {
                ...
                return (
                  <li key={p.id}><Link href={buildPathUrl}><a>{capitalize(p.goalName)}</a></Link></li>
                )
              })}
            ...
            <ul>
              {profile.pathsTaken.map((p) => {
                ...
                return (
                  <li key={p.id}><Link href={takePathUrl}><a>{capitalize(p.goalName)}</a></Link></li>
                ) 
              })}
...
export async function getServerSideProps({ params }) {
  ...
  try {
    ...
    const profilePathsQuery = await firebase.firestore()
      .collection('paths')
    ...
    const profilePathsTakenQuery = await firebase.firestore()
